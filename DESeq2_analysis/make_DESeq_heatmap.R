...
option_list = list(
  make_option(c("-I", "--input"), type="character", default="/home/drkksharma/SAMSA2/heatmap/",
              help="Input directory", metavar="character"),
  make_option(c("-N", "--entryCount"), type="integer", default=40,
              help="Number of row entries [default= %default]", metavar="character"),
  make_option(c("-O", "--out"), type="character", default="DESeq_heatmap-3.png", 
...
op <- par(family = "serif")
heatmap <- pheatmap(matrix, cluster_cols=FALSE, fontsize = 7, gaps_col=18, treeheight_row=5)

# saving and finishing up
cat ("Saving heatmap as ", save_filename, " now.\n")
png(file = save_filename, width=9, height=10,units='in', res=700 )
op <- par(family = "serif")
heatmap
dev.off()
