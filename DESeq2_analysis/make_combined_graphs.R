...
option_list = list(
  ...
  make_option(c("-O", "--out"), type="character", default="/home/drkksharma/SAMSA2/combined_graph.png",
              help="output file name [default= %default]", metavar="character"),
  make_option(c("-D", "--directory"), type="character", default="/home/drkksharma/SAMSA2/organism/",
...
exp_names_trimmed = paste("UC.", exp_names_trimmed, sep="")
...
for (x in control_files) {
  ...
  if (y == 1) {
    ...
    control_table[- grep("hypothetical", control_table$V3),]
...
for (x in exp_files) {
  ...
  if (y == 1) {
    ...
    exp_table[- grep("hypothetical", exp_table$V3),]
...
#full_table_top30<-full_table_top30[- grep("hypothetical", full_table_top30$Genus, invert = TRUE),]
# color palette
...
png(file = save_filename, width=8, height=10, units= 'in', res= 800)
op <- par(family = "serif")
grid.arrange(org_relative_ggplot, org_absolute_ggplot, ncol = 1)
dev.off()
